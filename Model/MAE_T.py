...
class TFR(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((cls_tokens[:, 0:1, :],patch2seq(self.cnn1(x))),dim=1)
        ...
        # t_token= x[:, t_token_idx]
        # avg = (t_token + m_token + p_token) / 3
        return x[:, t_token_idx]
...
class CRT(nn.Module):
    def __init__(
        ...
        self.mask_token = nn.Parameter(torch.randn(1, decoder_dim))
    ...
    #     mask = (1 - torch.eye(D)).unsqueeze(0).to(tokens.device)
    #     cross_mul = cross_mul * mask
    #     return torch.log(cross_mul.sum(-1).sum(-1)).mean(-1)
    ...
    def forward(self, x, mask_ratio=0.75, beta = 1e-4):
        ...
        # print(num_masked, num_patches,mask_ratio)
        # masked_indices1: masked index of temporal features
        # masked_indices2: masked index of spectral features
        rand_indices1 = torch.randperm(num_patches, device=device)
        masked_indices1 = rand_indices1[: num_masked].sort()[0]
        unmasked_indices1 = rand_indices1[num_masked:].sort()[0]
        ...
        unmasked_num_t= unmasked_indices1.shape[0]

        # t, m, p refer to temporal, magnitude, phase
        tpatches = patches[:, : num_patches, :, :]
        ...
        # print(t_tokens.shape,cls_tokens.shape)
        tokens = torch.cat((cls_tokens[:, 0:1, :], t_tokens),dim=1)
        # 3. Generate Positional Embeddings.
        # print(num_patches)
        t_idx = num_patches - 1
        # print(self.encoder.pos_embedding.shape)
        pos_embedding = torch.cat((self.encoder.pos_embedding[:, 0:1, :], self.encoder.pos_embedding[:, unmasked_indices1 + 1, :],self.encoder.pos_embedding[:, t_idx + 2: t_idx + 3],), dim=1)
        ...
        # # print(modal_embedding.shape,tokens.shape,pos_embedding.shape)
        tokens = tokens + pos_embedding

        encoded_tokens = self.encoder.transformer(tokens)

        t_idx = unmasked_num_t
        idc_loss = self.IDC_loss(self.projs[0](ori_tokens), self.projs[1](encoded_tokens[:, 1: t_idx+1]))
        # idc_loss = 0.5*self.IDC_loss(self.projs[0](ori_tokens), self.projs[1](encoded_tokens[:, 1: t_idx+1]))+self.IDC_loss2(self.projs[0](ori_tokens), self.projs[1](encoded_tokens[:, 1: t_idx+1]))
        ...
        recon_loss = F.mse_loss(pred_pixel_values_t, rearrange(patches[:,rand_indices], 'b n c p -> b n (c p)'))
        # print(recon_loss, idc_loss)
#         print(float(recon_loss), '....', float(info_loss))
        return recon_loss + idc_loss #0.9*
